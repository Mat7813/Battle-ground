...
void charger_niveau(char *nom, wave *vagues){ //charge le niveau à partir d'un fichier txt
...
int etat_partie_survivant(wave *vagues, joueur *player){//retourne l'état de la partie (0 si elle est toujours en cours, 1 si le joueur a gagné, -1 si il a perdu)
  if(player->pv==0)return -1; //si la vie du joueur est à 0, il a perdu
  else {
    int i, j=0;
    for(i=0;i<V;i++){ //on teste pour chaque vague si le nombre d'entité est égal à 0. Si c'est le cas cela veut dire que le joueur a survécu à toutes les vagues et qu'il a gagné
      if(vagues[i].nb==0)j++;
    }
    return(j==i);// si j est egal à i cela veut dire que pour chaques vagues il n'ya plus d'entites restantes donc le joueur a gagné
  }
}
