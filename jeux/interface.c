  ...
  while(jouer){
    ...
    if(event->type==SDL_QUIT){ //lorsque l'utilisateur appuie sur la croix rouge de la fenetre
      ...
      charger_image("data/menu/jouer/survieover.png", rendu, image, texture, rect,menu_x,150);
        if(event->type==SDL_MOUSEBUTTONDOWN){
           if(menu_jouer_difficulte(window, rendu, image, texture, rect, event, 2)==-1)return -1; //si la fonction retourne -1 c'est que l'utilisateur a appuyé sur la croix rouge. on retourne donc -1
        }
    }
