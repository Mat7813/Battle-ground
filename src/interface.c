...
/**
 * \fn int charger_image_option(char *nom, SDL_Renderer *rendu, int x, int y, int option)
 ...
 * \param char *nom, SDL_Renderer *rendu, int x, int y
...
int sous_menu_jouer(SDL_Window *window, SDL_Renderer *rendu, SDL_Event *event)
{
    ...
    while (jouer)
    {
        ...
        { // on réécoute les évènements mais avec un pointeur sur event car en SDL on ne peut pas faire plusieurs listener d'évènements.
            ...
            else if ((event->motion.x >= menu_x && event->motion.x <= 530) && (event->motion.y >= 250 && event->motion.y <= 300))
            { // pour les trois conditions qui suivent : si l'utilisateur passe sa souris dans la zone d'un bouton (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
...
void menu(SDL_Window *window, SDL_Renderer *rendu)
{
    ...
    charger_image("data/backgrounds/bgmenu1.bmp", rendu, 0, 0, 1); // on charge le fond sur lequel on va charger le menu
