/**
 * \file interface.c
 * \brief Contient toutes les fonctions utiles à l'interface graphique principale du jeu (gestion des menus, fonction de
 * chargement des images etc...) \author Lazare Maclouf \version 1 \date 22/01/2022
 */
#include "SDL2/SDL.h"
 ...
 * \brief Fonction qui sert à savoir quels chiffres composent le budget du joueur pour les afficher à l'écran (pour 1000
 * par exemple on aura 1.png, 0.png, 0.png, 0.png) \param joueur *p
 */
    ...
    if (p != NULL)
    {
        ...
        for (i = 0; nb2 > 10; i++)
        {
            ...
            case 0:
                ...
                break;
            case 1:
                strcpy(p->argent_img[i], "data/inventaire/1.png");
                break;
            case 2:
                strcpy(p->argent_img[i], "data/inventaire/2.png");
                break;
            case 3:
                strcpy(p->argent_img[i], "data/inventaire/3.png");
                break;
            case 4:
                strcpy(p->argent_img[i], "data/inventaire/4.png");
                break;
            case 5:
                strcpy(p->argent_img[i], "data/inventaire/5.png");
                break;
            case 6:
                strcpy(p->argent_img[i], "data/inventaire/6.png");
                break;
            case 7:
                strcpy(p->argent_img[i], "data/inventaire/7.png");
                break;
            case 8:
                strcpy(p->argent_img[i], "data/inventaire/8.png");
                break;
            case 9:
                strcpy(p->argent_img[i], "data/inventaire/9.png");
                break;
            default:
                break;
            }
...
void dessiner_rectangle_vide(SDL_Renderer *rendu, int code_couleur[4], int x, int y, int h, int w, int mode)
{
    ...
    if (mode)
        SDL_RenderPresent(rendu); // si le mode choisi est 1 on présente le rendu à l'écran
}
/**
 ...
 * \brief  sert à charger une image avec tous les formats (grâce à SDL_image) sur le rendu et à présenter le rendu sur
 * la fenetre (le format géré par défaut par la SDL est BMP) \param char *nom, SDL_Renderer *rendu, int x, int y, int
 * option \return -1 si un problème est survenu rien sinon
 */
int charger_image(char *nom, SDL_Renderer *rendu, int x, int y, int option)
{ // cette fonction sert à charger une image avec tous les formats (grâce à SDL_image) sur le rendu et à présenter le
  // rendu sur la fenetre (le format géré par défaut par la SDL est BMP)
    ...
    if (image == NULL)
        return -1;          // si l'image n'existe pas on retourne -1
    ...
    if (SDL_RenderCopy(rendu, texture, NULL, &rect) != 0)
        return -1; // on copie la texture dans le rendu
    if (option == 1)
        SDL_RenderPresent(rendu); // on a l'option de présentation du rendu ou non pour éviter les beug d'affichage on
                                  // superpose tout avant d'afficher
    SDL_DestroyTexture(texture);
    texture = NULL, image = NULL;
    return 0;
}
/**
 * \fn int charger_partie_image(char *nom, SDL_Renderer *rendu, int x, int y, int w, int h, int w_image, int h_image,
 * int x_image, int y_image, int option) \brief  sert à charger un morceau d'une image (la taille de la figure à rogner,
 * sa position et autres passés en paramètre) avec tous les formats (grâce à SDL_image) sur le rendu et à présenter le
 * rendu sur la fenetre (le format géré par défaut par la SDL est BMP) \param char *nom, SDL_Renderer *rendu, int x, int
 * y, int w, int h, int w_image, int h_image, int x_image, int y_image, int option \return -1 si un problème est survenu
 * rien sinon
 */
int charger_partie_image(char *nom, SDL_Renderer *rendu, int x, int y, int w, int h, int w_image, int h_image,
                         int x_image, int y_image, int option)
{ // cette fonction sert à charger une image avec tous les formats (grâce à SDL_image) sur le rendu et à présenter le
  // rendu sur la fenetre (le format géré par défaut par la SDL est BMP)
    SDL_Surface *image = NULL;   // pointeur pour une surface qu'on pourra par la suite charger sur le rendu
    SDL_Texture *texture = NULL; // pointeur sur la texture qu'on va manipuler avec le rendu
    ...
    SDL_Rect taille;
    taille.w = w_image, taille.h = h_image, taille.x = x_image,
    taille.y = y_image;    // curseur de sélection pour l'image à afficher
    image = IMG_Load(nom); // on commence par charger dans le pointeur sur la surface l'image BMP
    texture = SDL_CreateTextureFromSurface(rendu, image); // on crée une texture à partir du rendu et de l'image
    if (image == NULL)
        return -1;          // si l'image n'existe pas on retourne -1
    ...
    while (jouer)
    {
        ...
        { // on réécoute les évènements mais avec un pointeur sur event car en SDL on ne peut pas faire plusieurs
          // listener d'évènements.
            ...
            else if ((event->motion.x >= menu_x && event->motion.x <= 520) &&
                     (event->motion.y >= 150 && event->motion.y <= 200))
            ...
            else if ((event->motion.x >= menu_x && event->motion.x <= 530) &&
              ...
              // (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
                ...
                if (mode == 1)
                    if (jeu_classique(window, rendu, event, 2) == -1)
                     ...
                     (event->motion.y >= 290 && event->motion.y <= 340))
            { // pour les trois conditions qui suivent : si l'utilisateur passe sa souris dans la zone d'un bouton
              // (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
                ...
                if (mode == 1)
                    if (jeu_classique(window, rendu, event, 3) == -1)
                     ...
                     (event->motion.y >= 360 && event->motion.y <= 410))
            { // pour les trois conditions qui suivent : si l'utilisateur passe sa souris dans la zone d'un bouton
              // (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
 ...
 * \brief fonction servant à afficher les options de jeu et à charger les autres fonctions en conséquence (mode
 * classique, mode suvivant). \param SDL_Window *window, SDL_Renderer *rendu, SDL_Event *event \return -1 ou 0
 */
    ...
    while (jouer)
    {
        ...
        { // on réécoute les évènements mais avec un pointeur sur event car en SDL on ne peut pas faire plusieurs
          // listener d'évènements.
            ...
            {
                case_level++;
                case_level = case_level %3;
                case_level ++;
                fprintf(stderr, "%d", case_level);
            }
            ...
            if (event->type == SDL_QUIT || event->key.keysym.sym == SDLK_q)             // lorsque l'utilisateur appuie sur la croix rouge de la fenetre
                return -1; // on retourne -1
            ...
            { // pour les trois conditions qui suivent : si l'utilisateur passe sa souris dans la zone d'un bouton
              // (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
                if(case_level == 1){
                    charger_image("data/menu/jouer/classique.png", rendu, menu_x, 250, 1);
                    charger_image("data/menu/jouer/retour.png", rendu, menu_x, 350, 1);
                }
                charger_image("data/menu/jouer/survieover.png", rendu, menu_x, 150, 1);
                if (event->type == SDL_MOUSEBUTTONDOWN)
                {
                    if (jeu_survivant(window, rendu, event) == -1)
                        return -1; // si la fonction retourne -1 c'est que l'utilisateur a appuyé sur la croix rouge. on
                                   // retourne donc -1
                    else
                        charger_image("data/backgrounds/bgmenu1.bmp", rendu, 0, 0, 1);
                }
                     ...
                     (event->motion.y >= 250 && event->motion.y <= 300)
                     ||
                     case_level == 2)
            { 
              if(case_level == 2){
              ...
              // (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
                ...
                charger_image("data/menu/jouer/classiqueover.png", rendu, menu_x, 250, 1);
                if (event->type == SDL_MOUSEBUTTONDOWN)
                {
            ...
            { // pour les trois conditions qui suivent : si l'utilisateur passe sa souris dans la zone d'un bouton
              // (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
                charger_image("data/menu/jouer/retourover.png", rendu, menu_x, 350, 1);
                if (event->type == SDL_MOUSEBUTTONDOWN)
                {
                    charger_image("data/backgrounds/bgmenu1.bmp", rendu, 0, 0,
                                  1); // on recharge le décors pour effacer les boutons du menu actuel
                    jouer = SDL_FALSE;
                }
            }
            else
 ...
 * \brief fonction servant à afficher le menu principal du jeu avec ses différentes options (jouer, paramètres, quitter)
 * et à agir en fonction du choix de l'utilisateur. \param SDL_Window *window, SDL_Renderer *rendu
 */
void menu(SDL_Window *window, SDL_Renderer *rendu)
{
    ...
    if (SDL_Init(SDL_INIT_EVERYTHING) != 0)
        printf("erreur de chargement video"); // on charge toutes les fonctionnalités de la sdl
    ...
    while (programme)
    {
        while (SDL_PollEvent(&event))
        {
            ...
            if ((event.motion.x >= menu_x && event.motion.x <= 530) && (event.motion.y >= 150 && event.motion.y <= 200))
            { // pour les trois conditions qui suivent : si l'utilisateur passe sa souris dans la zone d'un bouton
              // (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
            ...
            else if ((event.motion.x >= menu_x && event.motion.x <= 730) &&
                     (event.motion.y >= 250 && event.motion.y <= 300))
            {
            ...
            { // pour éviter que tout soit dérangé lorsu'on redimmensionne la fenetre on recharge le décors principal et
              // on redimensionne la fentre de la meme taille qu'au début (dans le menu principal l'utilisateur ne peut
              // pas changer la taille de la fenetre)
 ...
 * \brief fonction principale du jeu. Elle est chargée de la Création et du lancement de la fenêtre du jeu, du rendu
 * ainsi que de l'appel des autres fonctions pour jouer. Enfin, elle s'occupe de détruire le rendu, la fenêtre et de
 * quitter la sdl une fois sorti des fonction de jeu (juste avant de quitter le programme) \param void \return 0
 */
int initialise_jeu()
{ // fonction servant à initialiser completement le jeu en appelant les fonctions principales d'affichages qui
  // elles-mêmes appeleront toutes les autres sous fonctions pour le jeu
    SDL_Window *window =
        NULL; // pointeur pour la fentre (ici on travaille uniquement avec des pointeurs pour que toutes les variables
              // soient accessibles partout dans le code et modifiables par d'autres fonctions facilement)
    SDL_Renderer *rendu = NULL; // pointeur rendu
    window = SDL_CreateWindow("Battle Ground", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1000, 564,
                              SDL_WINDOW_RESIZABLE); // on crée une fenetre de type "window_resizable" car par la suite
                                                     // on pourra agrandir la fenêtre
    ...
    menu(window, rendu); // fonction principale qui charge le menu. en parametre on passe tous les pointeurs qu'on a
                         // créé dans la fonction.
