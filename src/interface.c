...
{                              // cette fonction sert à charger une image avec tous les formats (grâce à SDL_image) sur le rendu et à présenter le rendu sur la fenetre (le format géré par défaut par la SDL est BMP)
...
{ // mode correspond au mode de jeu (1 pour classique et 2 pour survie)
  ...
  while (jouer)
  {
    ...
    { // on réécoute les évènements mais avec un pointeur sur event car en SDL on ne peut pas faire plusieurs listener d'évènements.
      ...
      else if ((event->motion.x >= menu_x && event->motion.x <= 520) && (event->motion.y >= 150 && event->motion.y <= 200))
      ...
      else if ((event->motion.x >= menu_x && event->motion.x <= 530) && (event->motion.y >= 220 && event->motion.y <= 270))
      { // pour les trois conditions qui suivent : si l'utilisateur passe sa souris dans la zone d'un bouton (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
      ...
      else if ((event->motion.x >= menu_x && event->motion.x <= 530) && (event->motion.y >= 290 && event->motion.y <= 340))
      { // pour les trois conditions qui suivent : si l'utilisateur passe sa souris dans la zone d'un bouton (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
      ...
      else if ((event->motion.x >= menu_x && event->motion.x <= 530) && (event->motion.y >= 360 && event->motion.y <= 410))
      { // pour les trois conditions qui suivent : si l'utilisateur passe sa souris dans la zone d'un bouton (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
...
int select_multi(SDL_Window *window, SDL_Renderer *rendu, SDL_Event *event)
{
  ...
  while (jouer)
  {
    ...
    { // on réécoute les évènements mais avec un pointeur sur event car en SDL on ne peut pas faire plusieurs listener d'évènements.
      ...
      else if ((event->motion.x >= menu_x && event->motion.x <= 530) && (event->motion.y >= 150 && event->motion.y <= 200))
      { // pour les trois conditions qui suivent : si l'utilisateur passe sa souris dans la zone d'un bouton (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
      ...
      else if ((event->motion.x >= menu_x && event->motion.x <= 530) && (event->motion.y >= 225 && event->motion.y <= 275))
      { // pour les trois conditions qui suivent : si l'utilisateur passe sa souris dans la zone d'un bouton (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
        charger_image("data/menu/hoteover.png", rendu, menu_x, 150, 1);
        if (event->type == SDL_MOUSEBUTTONDOWN)
        {
      ...
      else if ((event->motion.x >= menu_x && event->motion.x <= 530) && (event->motion.y >= 375 && event->motion.y <= 425))
      { // pour les trois conditions qui suivent : si l'utilisateur passe sa souris dans la zone d'un bouton (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
...
int sous_menu_jouer(SDL_Window *window, SDL_Renderer *rendu, SDL_Event *event)
{
  ...
  while (jouer)
  {
    ...
    {                              // on réécoute les évènements mais avec un pointeur sur event car en SDL on ne peut pas faire plusieurs listener d'évènements.
      ...
      else if ((event->motion.x >= menu_x && event->motion.x <= 530) && (event->motion.y >= 300 && event->motion.y <= 350))
      { // pour les trois conditions qui suivent : si l'utilisateur passe sa souris dans la zone d'un bouton (jouer, paramètres ou quitter), on change le fond du bouton en chargeant la version "over" de celui-ci
...
void menu(SDL_Window *window, SDL_Renderer *rendu)
{
  ...
  while (programme)
  {
    while (SDL_PollEvent(&event))
    {
      ...
      { // pour éviter que tout soit dérangé lorsu'on redimmensionne la fenetre on recharge le décors principal et on redimensionne la fentre de la meme taille qu'au début (dans le menu principal l'utilisateur ne peut pas changer la taille de la fenetre)
