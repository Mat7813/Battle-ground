...
void attaque_entites(t_wave *vague_ennemie, t_wave *vague_joueur, joueur *player){
  ...
  if(vague_ennemie->ent->x<=vague_joueur->ent->x+60&&vague_joueur->ent->type==2){
    ...
    vague_joueur->ent->montant=2;
    if(vague_ennemie->ent->x>vague_joueur->ent->x+60)vague_ennemie->ent->montant=1;
    if(!vague_joueur->ent->attaque){
      vague_ennemie->ent->pv-=vague_joueur->ent->degat;
      vague_joueur->ent->attaque=20;
    }
    ...
    if(vague_ennemie!=NULL)if(vague_ennemie->ent->montant==2)vague_ennemie->ent->montant=1;
    if(vague_joueur!=NULL)if(vague_joueur->ent->montant==2)vague_joueur->ent->montant=1;
  }
...
void gestion_environnement(t_wave *vague_ennemie, t_wave *vague_joueur, joueur *player, SDL_Renderer *rendu){ //fonction qui vérifie qu'un élément ne rentre pas en contact avec un autre
  ...
  if(vague_ennemie->ent->x<=400){
    if(player->def!=NULL&&player->t==NULL){
      ...
      else player->def->temps--;
    }
    }
    else if(player->def!=NULL)player->def->temps=0;
  }
  else if(player->def!=NULL)player->def->temps=0;
