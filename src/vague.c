...
void met_a_jour_images_separees(entite *ent)
{
  if (ent != NULL)
  {
    ...
    else
    {
      ...
      {
        ent->nom_fichier[25]++;
        if (!fichier_existe(ent->nom_fichier))
          ent->nom_fichier[25]--;
      }
  ...
  if (ent != NULL)
  {
    if (!ent->attaque || ent->attaque > 15)
    {
      if (!ent->nom_fichier_attaque[0])
      {
        ...
        if (!ent->y_image)
          ent->y_image += ent->h_image;
        if (ent->x_image / ent->nb_pos_attaque >= ent->w_image)
          ent->x_image = 0;
        else
          ent->x_image += ent->w_image;
      }
      else if (ent->nom_fichier_attaque[0] > 0)
    ...
    else if (!ent->nom_fichier_attaque[0] && ent->montant != 2)
    {
      ...
      if (ent->attaque)
      {
        if (ent->y_image > 0)
          ent->y_image = 0;
        if (ent->x_image / ent->nb_pos >= ent->w_image)
          ent->x_image = 0; /*si on a atteint le bord du fichier on remet x_image à 0*/
        else
          ent->x_image += ent->w_image;
      }
      if (ent->attaque > 1)
...
void charger_img_separees(entite *ent, SDL_Renderer *rendu)
{
  if (!ent->attaque || ent->attaque == 20 || ent->attaque == 19 || ent->attaque == 18)
    ...
    charger_image(ent->nom_fichier, rendu, ent->x, ent->y, 0);
}
