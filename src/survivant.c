...
#define argent_joueur 1000
    ...
    * \brief fonction qui sert à charger un niveau en mode survivant, en allant chercher dans un fichier dont le nom est passé en paramètres les informations du niveau (pour chaque vague le nombres d'entités, leur types, leur pv, le temps entre les vague etc..).
    * \param char *nom
    * \return t_wave * (pointeur sur t_wave qui aura été rempli par les informations contenu dans le fichier.)
    */
t_wave* charger_niveau(char *nom){ //charge le niveau à partir d'un fichier txt
  ...
  if(vague==NULL)return NULL;
    while(!feof(fichier)){
    fscanf(fichier, "%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d", &vague->ent->pv, &vague->ent->montant, &vague->ent->x, &vague->ent->x_barre, &vague->ent->y_barre, &vague->ent->degat, &vague->ent->w, &vague->ent->h, &vague->ent->x_image, &vague->ent->y_image, &vague->ent->w_image, &vague->ent->h_image,&vague->ent->nb_pos, &vague->ent->nb_pos_attaque);
    strcpy(vague->ent->nom_fichier, nom_template);
    strcpy(vague->ent->nom_fichier_attaque, nom_template_attaque);
    if(!vague->ent->pv){ /*si pv==0 alors cela veut dire qu'on ajoute du temps d'attente entre la prochaine vague*/
      temps+=600;
      vague->ent->temps=temps;
      vague->ent->pv=50; /*on remet pv à 50 (par défaut)*/
    }
    else vague->ent->temps=temps;
    ...
    if(vague->ent->w_image!=0){
      ...
      vague->ent->nom_fichier_attaque[0]=0; /*servira à réguler l'animation pour les entités ayant ce fichier à NULL*/
    }
    else {
 ...
 * \brief fonction qui sert à mettre automatiquement les images des entités à jour ainsi que d'autres champs (points de vie, position etc...).
   ...
   if(vague!=NULL){
     while(vague->suiv!=NULL){
      if(!vague->ent->temps&&vague->ent->pv>0){
       if(vague->ent->attaque!=0)vague->ent->charger_img(vague->ent, rendu);
       ...
       if(camp&&vague->ent->montant!=2){
        ...
        if(deb_liste_survivant(vague)==vague&&!camp){
            player->create=1;
          if(vague->ent->temps<=50)player->x_create+=5;
          else if(vague->ent->temps<=0){
            player->create=0;
            player->x_create=0;
          }
         ...
         if(vague->prec->ent->temps==0&&!camp){
          player->create=1;
          if(vague->ent->temps<=50)if(player->x_create<=265)player->x_create+=5;
        ...
        else if(vague->ent->temps<=50)if(player->x_create<=265)player->x_create+=5;
        }
        ...
        else player->create=0;
        vague=suivant_entite_survivant(vague);
      }
      if(vague->ent->pv<=0){
        vague=supprimer_entite_survivant(vague);
        if(camp)player->argent+=250;
        if(vague==NULL)return -1;
      }
      else if(vague->ent->pv>0&&!vague->ent->temps)vague=suivant_entite_survivant(vague);
   }
  }
  if(vague!=NULL){ //dans le cas ou il ne reste plus qu'une entité dans la liste ou on est à la fin
    if(!vague->ent->temps&&vague->ent->pv>0){
     if(vague->ent->attaque!=0)vague->ent->charger_img(vague->ent, rendu);
     vague->ent->met_a_jour(vague->ent);
     dessiner_rectangle_vide(rendu, code_contour_barre_vie, vague->ent->x_barre, vague->ent->y_barre, 8, 52, 0); /*met à jour la barre de vie*/
    ...
    if(vague->ent->temps>0){
      ...
      if(vague->prec!=NULL)if(vague->prec->ent->temps==0&&!camp){
        player->create=1;
        if(vague->ent->temps<=50)if(player->x_create<=265)player->x_create+=5;
        if(vague->ent->temps<=0){
          player->create=0;
 ...
 * \brief fonction qui sert à vérifier si la partie est finie ou si elle est toujours en cours (et si elle est finie si le joueur a gagné ou perdu)
 * \param t_wave *vague, joueur *player
 * \return int
 */
int etat_partie_survivant(t_wave *vague, joueur *player){//retourne l'état de la partie (0 si elle est toujours en cours, 2 si le joueur a gagné, -1 si il a perdu)
  if(player!=NULL){
    if(player->pv>0){
        if(vague==NULL){
          printf("gagné\n");
 ...
 * \brief fonction qui sert à afficher un message de prévention lorsque l'utilisateur veut quitter une partie lancée et en fonction du retour de ce dernier (oui, non ou annuler) retourner une valeur entière
 * \param void
 * \return int
 */
 ...
 * \brief Comme son nom l'indique, cette fonction démarre le mode survivant. C'est la fonction principale du mode jeu survivant et s'occupe de tout gérer (les évènements, l'argent et les unités de l'utilisateur; charger le niveau et toutes les entités qui vont avec etc/..)
 * \param SDL_Window *window, SDL_Renderer *rendu, SDL_Event *event, char lvl
 * \return int
 */
int demarrer_survivant(SDL_Window *window, SDL_Renderer *rendu, SDL_Event *event, char lvl){ //crée un ordinateur avec le nombre de vague liées au niveau.
    ...
    //  if(etat_partie_survivant(vague, &player)==1)return 1;//on verifie à chaque tour de boucle l'état de la partie
    //  if(etat_partie_survivant(vague, &player)==-1)return 0;
      ...
      if(compter_elem(vague_ennemies)>=20)message_box(&msg, "data/inventaire/derniere.png");
      if((event->motion.x>=620&&event->motion.x<=670)&&(event->motion.y>=10&&event->motion.y<=50)){
        ...
        indic=2;
          if(event->type==SDL_MOUSEBUTTONDOWN)vague_joueur=ajouter_fighter(player, vague_joueur, &msg);
        }
          ...
          if(event->type==SDL_MOUSEBUTTONDOWN)vague_joueur=ajouter_bandit(player, vague_joueur, &msg);
        indic=3;
        }
      ...
      if(msg.temps>0){
        if(fichier_existe(msg.nom_fichier))charger_image(msg.nom_fichier, rendu, msg.x, msg.y, 0);
        msg.temps--;
      }
   }
   if(msg.temps>0){
   ...
   if(!pause){
       ...
       if(indic!=2)charger_image("data/inventaire/carre2.png", rendu, 290, 10, 0);
       if(indic!=3)charger_image("data/inventaire/carre3.png", rendu, 360, 10, 0);
   ...
   if(!pause){
     if(deroulement_vague(player, vague_ennemies, rendu, 1)==-1)vague_ennemies=NULL;
     if(deroulement_vague(player, vague_joueur, rendu, 0)==-1)vague_joueur=NULL;
   }
