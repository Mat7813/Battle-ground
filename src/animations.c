...
void animation_attaque(SDL_Renderer *rendu, t_wave *vague){
  if(vague!=NULL){
    if(!vague->ent->attaque||vague->ent->attaque==20||vague->ent->attaque==19||vague->ent->attaque==18)charger_image(vague->ent->nom_fichier_attaque, rendu, vague->ent->x, vague->ent->y, 0);
    else return;
  }
}
